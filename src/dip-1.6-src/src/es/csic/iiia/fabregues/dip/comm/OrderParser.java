...
//			//TODO potser treure: tot això ho podrem acabar treient. Està aquí per si no és cert que sempre hagi de ser WVE el que faci llençar l'excepció. Amb un return false basta.
//			log.printError("Enlloc d'un order ha sortit això: "+order[6+index+1]+"\nSi no es tracte d'un WVE, ens hem de preocupar. Aqui va el text original:");
//			log.printError(order);
//			OrderType orderType = OrderType.valueOf(order[6+1]);
//			if(orderType!=OrderType.WVE){
//				System.exit(0);
//			}
		}
		return false;
	...
	/**
	 * Pot processar ordres en el següents formats:
	 * ( AUS AMY BUD ) HLD )
	 * SUB ( ( AUS AMY BUD ) HLD )
		...
		if(!orderMsg[0].equals("(")){ //power WVE
			...
			case MTO:
				...
				if(regionStr[0].equals("(")){
					...
					//busquem la regió de la provincia que tingui el tipus mateix de la regió que es mou
			...
			case WVE:
				//waive
				// No hauria d'arribar aquí, aquest cas es tracte més amunt perquè no té (region) al davant
				//log.printError("ups");
				break;
			default:
				//log.printError("ups");
